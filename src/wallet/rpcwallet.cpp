...
UniValue marmara_issue(const UniValue& params, bool fHelp)
{
    ...
    if (iter != keys.end()) {
        std::string value = jsonParams[iter - keys.begin()].get_str();}
		std::cerr << __func__ << " test output tcno=" << optParams.tcno << std::endl;

	iter = std::find(keys.begin(), keys.end(), "ad");
    if (iter != keys.end()) {
        std::string value = jsonParams[iter - keys.begin()].get_str();}
		std::cerr << __func__ << " test output ad=" << optParams.ad << std::endl;

	iter = std::find(keys.begin(), keys.end(), "soyad");
    if (iter != keys.end()) {
        std::string value = jsonParams[iter - keys.begin()].get_str();}
		std::cerr << __func__ << " test output soyad=" << optParams.soyad << std::endl;

	iter = std::find(keys.begin(), keys.end(), "dogumtarihi");
    if (iter != keys.end()) {
        std::string value = jsonParams[iter - keys.begin()].get_str();}
		std::cerr << __func__ << " test output dogumtarihi=" << optParams.dogumtarihi << std::endl;

...
CScript MarmaraEncodeLoopIssuerOpret(uint256 createtxid, CPubKey receiverpk, uint8_t autoSettlement, uint8_t autoInsurance, int32_t avalCount, tcno, ad, soyad, dogumtarihi, int32_t disputeExpiresHeight, uint8_t escrowOn, CAmount blockageAmount)
{
    CScript opret;
    uint8_t evalcode = EVAL_MARMARA;
...
CScript MarmaraEncodeLoopTransferOpret(uint256 createtxid, CPubKey receiverpk, int32_t avalCount, tcno, ad, soyad, dogumtarihi)
{
    CScript opret;
    uint8_t evalcode = EVAL_MARMARA;
...
UniValue MarmaraCreditloop(uint256 txid)
{
    ...
    if ((n = MarmaraGetbatontxid(creditloop, batontxid, txid)) > 0)
    {
        if (MarmaraGetLoopCreateData(creditloop[0], loopData) == 0)
        {
            ...
            result.push_back(pair("tcno",tcno))
            result.push_back(pair("ad",ad))
            result.push_back(pair("soyad",soyad))
            result.push_back(pair("dogumtarihi",dogumtarihi))
